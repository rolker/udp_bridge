...
|---------|----------|-------------------------------------|
| `udp_bridge` | C++17 (ament_cmake) | `udp_bridge` library, `udp_bridge_node` executable, 25 gtest targets (`utest`, `test_utilities`, `test_defragmenter`, `test_qos_resolution`, `test_qos_matching_integration`, `test_connection_rate_limit`, `test_remote_node_resend`, `test_connection_cleanup`, `test_unaddressed_connection`, `test_giveup_diagnostic`, `test_resend_budget`, `test_admission_control`, `test_publish_queue`, `test_subscriber_registry`, `test_stale_packet_gate`, `test_reorder_buffer`, `test_relay_routing`, `test_node_name_limits`, `test_relay_queue`, `test_drop_baseline`, `test_message_statistics`, `test_relay_send`, `test_admission_field_replay`, `test_message_atomicity`, `test_connection_diagnostic`) |
...
| `remotes_list` | `[]` | Each entry IS the remote's wire name (#67), not just a parameter-path label: your label for a peer must be the name that peer calls itself. An entry that is empty, over 23 chars, or equal to this bridge's own `name` fails `on_configure` (validated before the socket is opened; `""` is a reserved sentinel on the send path); a repeated entry is idempotent. The `add_remote` service applies the same over-long and self-name refusals at runtime (ERROR, no remote created). Then per-remote `remotes.<r>.connections_list`, per-connection `host`, `port`, `return_host`, `return_port`, `maximum_bytes_per_second` (0 → default **50000** B/s, `Connection::default_rate_limit`), `resend_budget_fraction` (0.25 — max fraction of measured **goodput** resends may consume; the basis moved from the admission cap to goodput in #52, #44, `doc/resend_budget_design.md`), `admission_floor_bytes_per_second` (**8192** B/s — absolute floor of the AIMD-adjusted admission cap, clamped at use to the connection's own `maximum_bytes_per_second`; negative/NaN fall back to the default; replaced the removed cap-relative `admission_floor_fraction` in #52. A floor at or above the connection's own `maximum_bytes_per_second` makes AIMD inert — the cap can never be reduced — and `on_configure` logs a WARN naming the connection when it is), `link_headroom_fraction` (**RETIRED (#52), a configure-time tripwire only**: it used to make a congested sample target `(1 − this) × goodput`; that clamp was removed on 2026-08-25 because goodput is depressed by the throttling the clamp computed. **All behaviour is gone** — no `Connection` state, no setter, no message field, no control-law read, and it is **no longer declared** as a parameter (a retired key must not appear in `ros2 param list` looking live). Detected by **PRESENCE** in the node's parameter overrides — which rclcpp carries whether or not a parameter is declared — so setting it to **any** value, *including the old `0.2` default*, logs a WARN at `on_configure` naming the connection. (Round 3 of #52: the earlier value-comparison form matched YAML `0.2` only by a `float`→`double` widening accident, and would have gone silent on the likeliest stale value if the constant were tidied to a `double`.) Unlike `remotes.<label>.name` it does **not** fail the transition — a stale headroom value misroutes nothing. **No headroom mechanism remains**: the refractory-gated multiplicative decrease protects a co-tenant only insofar as the bridge shares the loss, so on a saturated path where the bridge wins the queue it never backs off; a replacement basis is argued on #61. `doc/admission_control_design.md`), `admission_refractory_period_seconds` (**5.0** s, a double — an INTEGER literal such as `5` is now **coerced at the declaration site** (`declareDoubleIfMissing`, #52 round 3), so a params file written that way configures instead of failing the transition; the same coercion covers `resend_budget_fraction`, `admission_floor_bytes_per_second`, `reorder_hold_window_ms`, `resend_giveup_*_rate_per_s` and per-topic `period`. Minimum interval between admission-control decisions. Inside the window the controller FREEZES: no decrease and no additive recovery, whatever the sample says. Doubles per further decrease within an unresolved episode, resets on the first clean sample. 0 disables the gate — restoring the pre-#52 behaviour that caused the incident — and **logs a WARN** saying so (#52 round 3); negative/NaN fall back to the default; values above **60 s** (`kMaximumAdmissionRefractoryPeriodSeconds`, `+Inf` included) CLAMP to that ceiling rather than falling back, so a typo cannot switch the controller off for the life of the node. The grown window is bounded by 2× the base **and** by the same 60 s ceiling, so the freeze never exceeds 60 s at any configured base. Added in #52 after the 2026-08-25 Appledore collapse — one marginal delivery sample became seven halvings in 12 s, 35 times in 9.5 hours. Configure-time only, like every other admission parameter — see #75), `topics_list`, and per-topic `source` (default: topic label), `destination` (default: source), `queue_size` (10), `period` (0.0), `reliability`, `durability`, `history_depth` (0, clamped ≤ 10000) |
...
The per-connection status carries the AIMD state as KeyValues (#52):
`rate_limit_bytes_per_sec` (the CONFIGURED limit, which never moves)
alongside `effective_rate_limit_bytes_per_sec`,
`admission_refractory_window_s` and `admission_floor_bytes_per_sec`.
Its summary names the cap when one is in force and distinguishes
deliberate admission **shedding** from socket-level **tx failures** —
one shared "tx failures/drops" wording covered both through the
2026-08-25 incident. Level/summary logic:
