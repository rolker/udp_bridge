...
- **An `OnSetParameters` callback fires on `declare_parameter` too.**
  rclcpp calls it for declarations as well as sets (`rclcpp/node.hpp`), so a
  callback registered before `on_configure`'s `declareIfMissing` calls sees
  every YAML override on its way in — and *rejecting* one there throws
  `InvalidParameterValueException` out of `on_configure`, which
  rclcpp_lifecycle swallows, leaving a half-configured node with the socket
  already bound and no clear error. That is why
  `on_set_parameters_handle_` is acquired at the very END of
  `on_configure`, and why the runtime validation may be strict while
  `on_configure` keeps clamping mistyped config values. Do not move the
  registration earlier.
- **Runtime-tunable parameters are resolved through a built map, not by
  parsing the name.** `runtime_tunables_` maps the full parameter name to
  its target, recorded in `on_configure` where the name is constructed. A
  remote's wire name may itself contain `.` (#67), so
  `remotes.<r>.connections.<c>.<key>` is not unambiguously splittable, and
  a per-topic parameter path is spelled with the `topics_list` label while
  the routing table is keyed by the resolved `source`.
