...
bool Defragmenter::addFragment(std::vector<uint8_t> fragment, rclcpp::Time now)
{
  ...
  if(fragment_packet->type == PacketType::Fragment)
  {
    ...
    {
      fragment_map_[fragment_packet->packet_id].first_arrival_time = now;
...
int Defragmenter::cleanup(rclcpp::Time discard_time)
{
