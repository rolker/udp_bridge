...
-   `remotes.<remote_label>.connections.<connection_id>.link_headroom_fraction`: (double, 0.0–1.0, default 0.2) Fraction of measured goodput deliberately left unused so a co-tenant on the same path (SSH, operator management traffic) is not starved. Applied as the target of a congested backoff. See `doc/admission_control_design.md`.
