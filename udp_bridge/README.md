...
### Using Launch (Recommended)
The provided launch file simplifies startup by automatically transitioning the node to the `Active` state.
...
The node uses a nested parameter structure to define remotes and connections. It is highly recommended to use a YAML configuration file rather than setting these via the command line.

An example configuration file is provided in `config/example_params.yaml`.
...
- Dynamically publishes topics received from remotes.
...
Developer documentation can be generated with doxygen.
