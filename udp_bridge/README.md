...
-   `remotes.<remote_label>.connections.<connection_id>.link_headroom_fraction`: (double, `[0.0, 1.0)`, default 0.2; values are clamped to 0.99) Fraction of measured goodput deliberately left unused so a co-tenant on the same path (SSH, operator management traffic) is not starved. **Currently inert**: its only call site was the `(1 − headroom) × goodput` clamp on the congested backoff, removed in the 2026-08-25 pass because goodput is depressed by the throttling the clamp was computing. Still declared, clamped and reported so existing configs keep working, but nothing reads it; the co-tenant guarantee is now carried by the refractory-gated multiplicative decrease, verified against the bench's management-flow invariant. See `doc/admission_control_design.md`, "`link_headroom_fraction` after the clamp removal".
