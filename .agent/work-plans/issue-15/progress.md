...
### Actions
- [ ] **R1 (Q2 broke can_send's monotone-order assumption)** — The reserve-then-record pattern releases the reservation and adds the record AFTER `sendto` runs unlocked. Under concurrent sends, a slower-sending thread can append its record (with an older `ret.timestamp`) AFTER a faster-sending thread's record (with a newer timestamp), producing an out-of-order deque. `PacketSendStatistics::can_send` does a skip-prefix walk (`while(start->timestamp < one_second_ago) start++`) and then sums the rest — but it stops at the first NOT-old entry, so any older entry sitting after that point gets summed. Result: total_sent over-counts → can_send returns false more often than warranted → connection throttles below `maximum_bytes_per_second` under concurrent contention. Fix: drop the skip-prefix optimization; iterate the whole deque and filter by timestamp. The deque is bounded to ~10 s, so the perf delta is negligible.
- [ ] **R2 (test passes on drop-everything regression)** — `ConcurrentSendsStayUnderLimit` asserts only `success_bytes < cap` (trivially true if no sends succeed) and `dropped > 0` (true even at 100% drop rate). Add `EXPECT_GT(rates.success_bytes_per_second, 0.0f)` plus a near-cap lower bound (e.g., `EXPECT_GE(rates.success_bytes_per_second, 0.5f * kRateLimitBytesPerSec)`) to catch over-conservative regressions.
- [ ] **R3 (Q2 comment misrepresents call-site behavior)** — `connection.cpp:288-293` says the caller "is going to observe the exception and decide what to do". No caller in the workspace catches `ConnectionException` — outer `Connection::send` (`connection.cpp:225`) doesn't; `UDPBridge::callback` (`udp_bridge.cpp:1078`) doesn't. The throw actually propagates to the executor and likely terminates the node. Pre-existing behavior — Q2 didn't introduce it, but the comment in Q2's commit needs to reflect reality. Reword to say the RAII guard's value is consistent accounting in the throw-path (not delegating reporting to a caller); note that the node terminates on uncaught throws.
